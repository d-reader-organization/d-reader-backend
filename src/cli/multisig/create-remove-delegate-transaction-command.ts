...
@Command({
  name: 'create-remove-delegate-transaction',
  description: 'Create remove delegate transaction to be used in multisig',
})
export class CreateRemoveDelegateTransactionCommand extends CommandRunner {
  ...
  async run(_: string[], options: Options): Promise<void> {
    options = await this.inquirerService.ask(
      'create-remove-delegate-transaction',
      options,
    );
    await this.createRemoveDelegateTransactionCommand(options);
  }

  async createRemoveDelegateTransactionCommand(options: Options) {
    log("🏗️  Starting 'create-remove-delegate-transaction' command...");
    ...
    try {
      ...
      if (
        ...
        throw new BadRequestException(
          `No delegate ${delegate.toString()} exists for collection : ${collectionAddress.toString()}`,
        );
      }
      ...
      const transaction = await updateCollectionPlugin(umi, {
        ...
        plugin: {
          type: 'UpdateDelegate',
          additionalDelegates:
            collection.updateDelegate.additionalDelegates.filter(
              (value) => !isEqual(value, delegate),
            ),
        },

...
export async function mintInstruction(
  ...
  const nftTokenAccount = metaplex.tokens().pdas().associatedTokenAccount({mint : mint.publicKey, owner : payer})
  ...
  ixs.push(createMintToInstruction(mint.publicKey, nftTokenAccount, payer, 1, []));

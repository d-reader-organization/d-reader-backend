...
export const getComicsQuery = (query: ComicParams) => {
  ...
  return Prisma.sql`SELECT comic."title", comic.slug, comic."audienceType", comic.cover, comic.banner, comic.logo, comic.description, comic."flavorText", comic.website, comic.twitter, comic.discord, comic.telegram, comic.instagram, comic."tikTok", comic."youTube", comic."updatedAt", comic."createdAt", comic."featuredAt", comic."verifiedAt", comic."publishedAt", comic."popularizedAt", comic."completedAt", comic."isFreeToRead", json_agg(distinct genre.*) AS genres, to_jsonb(creator) as creator,
